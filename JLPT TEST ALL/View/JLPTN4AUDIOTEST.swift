...
struct JLPTN4AUDIOTEST: View {
    ...
    var body: some View {
        VStack(spacing: 10) {
            ...
            if !JLPTN4AUDIOTESTaudioQuestions.isEmpty, JLPTN4AUDIOTESTaudioQuestions.indices.contains(currentIndex) {
                QuestionView(audioQuestion: JLPTN4AUDIOTESTaudioQuestions[currentIndex], tappedAnswer: $tappedAnswer) { option in
                    guard tappedAnswer == "" else { return }
                    withAnimation(.easeInOut) {
                        tappedAnswer = option
                        
                        if JLPTN4AUDIOTESTaudioQuestions[currentIndex].answer == option {
                            score += 1.0
                        }
                    }
                }
            ...
            CustomButton(title: currentIndex == (JLPTN4AUDIOTESTaudioQuestions.count - 1) ? "End" : "Next Question") {
                // 오디오 정지
                audioPlayer?.stop()
                
                if currentIndex == (JLPTN4AUDIOTESTaudioQuestions.count - 1) {
                    ...
                    withAnimation(.easeInOut) {
                        currentIndex += 1
                        progress = CGFloat(currentIndex) / CGFloat(JLPTN4AUDIOTESTaudioQuestions.count - 1)
        ...
        .fullScreenCover(isPresented: $showScoreCard) {
            ...
            ScoreCardView(score: score / CGFloat(JLPTN4AUDIOTESTaudioQuestions.count) * 100) {
                dismiss()
                onFinish()
            }
        }
