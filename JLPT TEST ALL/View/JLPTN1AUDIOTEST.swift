...
struct JLPTN1AUDIOTEST: View {
    ...
    var body: some View {
        VStack(spacing: 10) {
            ...
            if !JLPTN1AUDIOTESTaudioQuestions.isEmpty, JLPTN1AUDIOTESTaudioQuestions.indices.contains(currentIndex) {
                QuestionView(audioQuestion: JLPTN1AUDIOTESTaudioQuestions[currentIndex], tappedAnswer: $tappedAnswer) { option in
                    guard tappedAnswer == "" else { return }
                    withAnimation(.easeInOut) {
                        tappedAnswer = option
                        
                        if JLPTN1AUDIOTESTaudioQuestions[currentIndex].answer == option {
                            score += 1.0
                        }
                    }
                }
            ...
            CustomButton(title: currentIndex == (JLPTN1AUDIOTESTaudioQuestions.count - 1) ? "End" : "Next Question") {
                // 오디오 정지
                audioPlayer?.stop()
                
                if currentIndex == (JLPTN1AUDIOTESTaudioQuestions.count - 1) {
                    ...
                    withAnimation(.easeInOut) {
                        currentIndex += 1
                        progress = CGFloat(currentIndex) / CGFloat(JLPTN1AUDIOTESTaudioQuestions.count - 1)
        ...
        .fullScreenCover(isPresented: $showScoreCard) {
            ...
            ScoreCardView(score: score / CGFloat(JLPTN1AUDIOTESTaudioQuestions.count) * 100) {
                dismiss()
                onFinish()
            }
        }
