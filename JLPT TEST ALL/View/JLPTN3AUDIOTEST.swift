...
struct JLPTN3AUDIOTEST: View {
    ...
    var body: some View {
        VStack(spacing: 10) {
            ...
            if !JLPTN3AUDIOTESTaudioQuestions.isEmpty, JLPTN3AUDIOTESTaudioQuestions.indices.contains(currentIndex) {
                QuestionView(audioQuestion: JLPTN3AUDIOTESTaudioQuestions[currentIndex], tappedAnswer: $tappedAnswer) { option in
                    guard tappedAnswer == "" else { return }
                    withAnimation(.easeInOut) {
                        tappedAnswer = option
                        
                        if JLPTN3AUDIOTESTaudioQuestions[currentIndex].answer == option {
                            score += 1.0
                        }
                    }
                }
            ...
            CustomButton(title: currentIndex == (JLPTN3AUDIOTESTaudioQuestions.count - 1) ? "End" : "Next Question") {
                // 오디오 정지
                audioPlayer?.stop()
                
                if currentIndex == (JLPTN3AUDIOTESTaudioQuestions.count - 1) {
                    ...
                    withAnimation(.easeInOut) {
                        currentIndex += 1
                        progress = CGFloat(currentIndex) / CGFloat(JLPTN3AUDIOTESTaudioQuestions.count - 1)
        ...
        .fullScreenCover(isPresented: $showScoreCard) {
            ...
            ScoreCardView(score: score / CGFloat(JLPTN3AUDIOTESTaudioQuestions.count) * 100) {
                dismiss()
                onFinish()
            }
        }
